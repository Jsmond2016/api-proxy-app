...
export function ResponsePayloadViewer(props: ResponsePayloadViewerProps) {
  ...
  function updateQuery(value: string) { setQuery(value); setMatchIndex(0); }
  ...
  function handleKeyDown(event: ReactKeyboardEvent<HTMLInputElement>) {
    ...
    event.preventDefault();
    if (event.shiftKey) { moveMatch(-1); return; }
    moveMatch(1);
  }
  ...
  return <div className="response-payload"><div className="response-payload-heading"><strong>响应内容</strong><div className="response-search-tools"><Input ref={searchRef} allowClear onChange={(event) => updateQuery(event.target.value)} onKeyDown={handleKeyDown} placeholder="搜索响应内容（Ctrl/Cmd+F）" prefix={<Search size={14} />} value={query} /><span className="response-match-count">{matchLabel(matchIndex, matchCount)}</span><Tooltip title="上一个匹配"><Button aria-label="上一个匹配" disabled={matchCount === 0} icon={<ChevronUp size={15} />} onClick={() => moveMatch(-1)} size="small" /></Tooltip><Tooltip title="下一个匹配"><Button aria-label="下一个匹配" disabled={matchCount === 0} icon={<ChevronDown size={15} />} onClick={() => moveMatch(1)} size="small" /></Tooltip></div></div><pre ref={contentRef}>{renderSearchableText(props.body, query, matchIndex)}</pre><FieldPath activePath={activePath} hasQuery={Boolean(query.trim())} onCopy={() => { void copyPath(); }} /></div>;
}
...
function FieldPath(props: { activePath: string; hasQuery: boolean; onCopy: () => void }) {
  ...
  return <div className="response-field-path"><span>字段路径：</span><code>{path}</code><Tooltip title="复制字段路径"><Button aria-label="复制字段路径" disabled={!props.activePath} icon={<Copy size={14} />} onClick={props.onCopy} size="small" type="text" /></Tooltip></div>;
}
