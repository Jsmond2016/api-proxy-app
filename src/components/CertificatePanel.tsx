...
export function CertificatePanel(props: CertificatePanelProps) {
  ...
  return (
    <>
      <Button
        className="config-entry-button"
        icon={<KeyRound size={15} />}
        onClick={() => setOpen(true)}
      >
        {certificateEntryLabel(props.certificate)}
      </Button>
      <Modal
        footer={null}
      ...
      >
        <section className="certificate-modal-content">
          <Alert
            description="微信开发者工具信任此根证书后，本地代理才能解密 HTTPS 请求、匹配 Mock 接口并转发到 Apifox。"
            message="HTTPS Mock 必需配置"
            showIcon
            type="info"
          />
          <div className="certificate-modal-status">
            <div className={iconClass(props.certificate)}>
              <KeyRound size={18} />
            </div>
            <div className="certificate-copy">
              <strong>{certificateTitle(props.certificate)}</strong>
              <small>{certificateDetail(props.certificate)}</small>
            </div>
          </div>
          <Descriptions bordered column={1} size="small">
            <Descriptions.Item label="信任状态">
              {certificateTrustLabel(props.certificate)}
            </Descriptions.Item>
            <Descriptions.Item label="SHA-256 指纹">
              {props.certificate.fingerprint || "尚未生成"}
            </Descriptions.Item>
            <Descriptions.Item label="证书路径">
              {props.certificate.certificatePath || "尚未生成"}
            </Descriptions.Item>
          </Descriptions>
          <div className="certificate-modal-actions">
            <Button icon={<KeyRound size={15} />} onClick={props.onGenerate} type="primary">
              生成证书
            </Button>
            <Button
              disabled={!props.certificate.generated}
...
function certificateEntryLabel(certificate: CertificateStatus) {
  if (certificate.trusted) return "HTTPS 证书 · 已信任";
  if (certificate.generated) return "HTTPS 证书 · 未信任";
  return "配置 HTTPS 证书";
}
function certificateTrustLabel(certificate: CertificateStatus) {
  if (certificate.trusted) return "已始终信任";
  if (certificate.generated) return "待导入或待信任";
  return "尚未生成";
}
