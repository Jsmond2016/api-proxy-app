            ...
            <Tooltip title="统一开启或关闭当前 Tab 的全部 Mock 接口">
              <Switch
                ...
                checked={
                  ...
                  props.profile.rules.every((rule) => rule.enabled)
                }
...
function RuleActions(props: {
    ...
    try {
      if (localResponse) {
        ...
        setResult({
          ...
          statusText: statusText(localResponse.status),
  ...
  return (
    <div className="row-actions">
      ...
      >
        {renderTestContent(testing, result, props.rule)}
      </Modal>
    </div>
  );
}
...
function renderTestContent(testing: boolean, result: TestResult | null, rule: ProxyRule) {
  if (testing)
    return (
  ...
  return renderTestResult(result, rule);
}
function renderTestResult(result: TestResult | null, rule: ProxyRule) {
  if (!result) return null;
  let message = result.statusText;
  let type: "success" | "error" = "error";
  if (result.status) message = `${result.status} ${result.statusText}`;
  if (result.status >= 200 && result.status < 300) type = "success";
  return (
    <div className="test-result">
      ...
      {renderTestPayload(result)}
    </div>
  );
}
function renderTestPayload(result: TestResult) {
  if (result.error)
    return (
      <div>
        <strong>错误信息</strong>
  ...
  return <ResponsePayloadViewer body={result.body || "（空响应）"} />;
}
