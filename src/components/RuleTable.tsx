...
export function RuleTable(props: RuleTableProps) {
  ...
  const columns = useMemo<TableColumnsType<ProxyRule>>(() => [
    ...
    { title: "接口信息", dataIndex: "name", width: 380, render: (_, rule) => <div className="rule-info-cell"><div className="rule-name-line">{rule.apifoxWebUrl ? <a className="rule-name-link" href={rule.apifoxWebUrl} onClick={(event) => { event.preventDefault(); void props.onOpenUrl(rule.apifoxWebUrl); }} title="在 Apifox Web 打开接口">{rule.name}</a> : <strong>{rule.name}</strong>}</div><span className="request-cell"><span className={`method-badge method-${rule.method.toLowerCase()}`}>{rule.method}</span><RulePath message={message} rule={rule} /></span></div> },
...
function RulePath(props: { rule: ProxyRule; message: ReturnType<typeof AntApp.useApp>["message"] }) {
  async function copyPath() {
    try {
      await navigator.clipboard.writeText(props.rule.path);
      props.message.success("复制成功");
    } catch {
      props.message.error("复制失败，请检查剪贴板权限");
    }
  }
  return <button aria-label="复制接口 URL" className="rule-path-link" onClick={() => { void copyPath(); }} title="点击复制接口 URL" type="button"><code>{props.rule.path}</code></button>;
}
