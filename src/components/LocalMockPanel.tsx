...
export function LocalMockPanel(props: LocalMockPanelProps) {
  ...
  return <><Button icon={<Plus size={15} />} onClick={() => setOpen(true)}>新建本地 Mock</Button><Modal destroyOnHidden footer={null} onCancel={() => setOpen(false)} open={open} title="本地 Mock 响应"><div className="local-mock-toolbar"><Button icon={<Plus size={15} />} onClick={startCreate} type="primary">新增</Button></div><Table columns={columns} dataSource={props.responses} locale={{ emptyText: <Empty description="暂无预设响应" image={Empty.PRESENTED_IMAGE_SIMPLE} /> }} pagination={false} rowKey="id" size="small" /><Modal destroyOnHidden onCancel={() => setFormOpen(false)} onOk={() => { void form.submit(); }} open={formOpen} title={formTitle}><Form form={form} layout="vertical" onFinish={submit} initialValues={{ delayMs: 0, status: 200 }}><Form.Item label="响应名字" name="name" rules={[{ required: true, message: "请输入响应名字" }]}><Input placeholder="例如：成功、失败、异常" /></Form.Item><div className="form-grid"><Form.Item label="延时返回（毫秒）" name="delayMs"><InputNumber min={0} max={60000} style={{ width: "100%" }} /></Form.Item><Form.Item label="HTTP 状态" name="status" rules={[{ required: true }]}><InputNumber min={100} max={599} style={{ width: "100%" }} /></Form.Item></div><Form.Item label="响应体" name="body"><Input.TextArea autoSize={{ minRows: 8, maxRows: 18 }} placeholder="粘贴 JSON 或文本响应" /></Form.Item></Form></Modal></Modal></>;
}
