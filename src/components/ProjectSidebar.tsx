...
export function ProjectSidebar(props: ProjectSidebarProps) {
  ...
  const tabItems = props.profiles.map((profile) => ({
  ...
  return (
    <header className="project-sidebar">
      <div className="project-tabs-bar">
        <Tabs
          activeKey={props.activeProfileId || undefined}
          items={tabItems}
          onChange={(id) => {
            void props.onSelect(id);
          }}
        />
        <div className="project-tabs-actions">
          <ProjectPresetPanel
            ...
            disabled={props.disabled}
            onCreate={props.onCreateFromPreset}
            onExport={props.onExportPreset}
            onRead={props.onReadPreset}
          />
          <Tooltip title="新建项目">
            <Button
              aria-label="新建项目"
          ...
          <Tooltip title="编辑当前项目">
            <Button
              aria-label="编辑当前项目"
              className="icon-button"
              disabled={!props.activeProfileId || props.disabled}
              icon={<Pencil size={16} />}
              onClick={openEdit}
              type="text"
            />
          </Tooltip>
          <Tooltip title="删除当前项目">
            <Button
              aria-label="删除当前项目"
              className="icon-button danger-utility"
              danger
              disabled={!props.activeProfileId || props.disabled || props.profiles.length <= 1}
              icon={<Trash2 size={16} />}
...
function DeleteProjectDialog(props: {
  ...
  return (
    ...
    >
      <p>
        确定删除“{props.profile.name}”及其本地 Mock 接口吗？该项目配置中的 Access Token 和 Mock
        Token 也会删除。
      </p>
    </Modal>
  );
...
function ProfileDialog(props: ProfileDialogProps) {
  ...
  return (
    <Modal
      ...
      destroyOnHidden
      okButtonProps={{
        disabled: !name.trim() || !hosts.trim() || Number(port) < 1 || Number(port) > 65535,
      }}
      okText="保存"
      onCancel={props.onClose}
      onOk={submit}
      open
      title={profileDialogTitle(props.profile)}
    >
